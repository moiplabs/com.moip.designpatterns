...
require_once 'com/moip/http/CookieManager.php';
...
class HTTPCookieManager implements CookieManager {
	...
	public function setCookie( $setCookie , $domain = null ) {
		if ( is_array( $setCookie ) ) {
			...
			if ( preg_match( '/(?<name>[^\=]+)\=(?<value>[^;]+)(; expires=(?<expires>[^;]+))?(; path=(?<path>[^;]+))?(; domain=(?<domain>[^;]+))?(; (?<secure>secure))?(; (?<httponly>httponly))?/' , $setCookie , $matches ) ){
				$cookieName = null;
				$cookieValue = null;
				$cookieExpires = INF;
				$cookiePath = '/';
				...
				$this->cookies[ $cookieDomain ][] = new Cookie( $cookieName , $cookieValue , $cookieDomain , $cookieExpires , $cookiePath , $cookieSecure );
			}
		}
	}
